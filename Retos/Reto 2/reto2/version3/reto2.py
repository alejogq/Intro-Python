...
def sumaDiagonalPrincipal(A,n):
    s = 0
    for i in range(0, n):
        s = s + A[i][i]
    return s

def productoDiagonalSecundaria(A,n):
    f = 1
    for i in range(0, n):
        f = f * A[i][n-i-1]
    return f
...
def productoDiagonalPrincipal(A,n):
    f = 1
    for i in range(0, n):
        f = f * A[i][i]
    return f
...
def solucion(A):
    ...
    sum_diagonal_principal = sumaDiagonalPrincipal(A,n)
    
    prod_diagonal_secundaria =  productoDiagonalSecundaria(A,n)
